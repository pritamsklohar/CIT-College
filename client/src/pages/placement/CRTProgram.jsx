    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="container mx-auto px-4 py-20">
        ...
        <div className="grid lg:grid-cols-2 gap-16 mb-24">
          ...
          {/* Aptitude & GD Section */}
          <div>
            <h2 className="text-3xl font-black text-primary mb-10 flex items-center gap-3 uppercase">
              <span className="w-2 h-10 bg-accent inline-block rounded-full"></span> Aptitude / GD
            </h2>
            ...
            <div className="space-y-8">
              {aptitudePrograms.map((prog, i) => (
                <motion.div 
                  ...
                  viewport={{ once: true }}
                  key={i} 
                  className="bg-gray-50 rounded-3xl p-8 border border-transparent hover:border-accent hover:shadow-xl transition group relative overflow-hidden"
                >
                  ...
                  <h3 className="text-xl font-black text-gray-800 mb-6 flex items-center gap-4 relative z-10">
                    <div className="w-12 h-12 bg-primary text-white rounded-xl flex items-center justify-center shrink-0 shadow-lg group-hover:bg-accent transition">
                      {prog.icon}
                    </div>
                    {prog.title}
                  </h3>
            ...
            <div className="mt-10 space-y-4">
               {additionalActivities.map((act, i) => (
                 <motion.div 
                   ...
                   viewport={{ once: true }}
                   key={i} 
                   className="bg-white p-5 rounded-2xl shadow-md border-l-4 border-primary flex items-center gap-4 hover:border-accent hover:shadow-lg transition group"
                 >
                   <div className="text-2xl text-accent group-hover:scale-110 transition shrink-0">{act.icon}</div>
                   <h4 className="font-bold text-gray-700">{act.title}</h4>
                 </motion.div>
               ))}
            </div>
          </div>
          ...
          <div className="lg:sticky lg:top-24 h-fit">
            <div className="bg-primary p-12 rounded-3xl text-white shadow-2xl relative overflow-hidden group">
               ...
               <h2 className="text-4xl font-black text-accent mb-8 flex items-center gap-4 relative z-10">
                 <FaBrain className="text-5xl shrink-0" />
                 Personality Development
               </h2>
               
               <div className="space-y-6 relative z-10 text-justify text-lg leading-relaxed text-gray-100">
                 <p>
                 ...
                 <p className="p-6 bg-white/10 rounded-2xl border-l-4 border-accent italic">
