      ...
      <div className="container mx-auto px-4 py-20">
        ...
        {loading ? (
          <div className="grid md:grid-cols-3 gap-8">
             {[1,2,3].map(i => <div key={i} className="h-80 bg-white animate-pulse rounded-3xl shadow-sm"></div>)}
          </div>
        ) : (
            ...
            {achievements.map((ach, index) => (
              ...
              >
                <div className="h-64 relative overflow-hidden">
                   ...
                   <div className="absolute top-4 left-4 bg-accent text-white p-3 rounded-2xl shadow-xl"><FaTrophy /></div>
                </div>
                <div className="p-8">
                   <div className="text-xs font-black text-accent uppercase tracking-[0.2em] mb-2 italic">{ach.category}</div>
                   <h3 className="text-xl font-bold text-primary mb-2">{ach.title}</h3>
                   <p className="text-sm font-bold text-gray-500 mb-4">— {ach.studentName}</p>
                   <p className="text-gray-600 text-sm leading-relaxed">{ach.description}</p>
                   <div className="mt-6 pt-6 border-t flex justify-between items-center">
                      <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">{new Date(ach.date).toLocaleDateString()}</span>
                      <div className="text-accent flex gap-1"><FaStar /><FaStar /><FaStar /></div>
                   </div>
                </div>
              </motion.div>
            ))}
