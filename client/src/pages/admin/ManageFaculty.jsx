...
const ManageFaculty = () => {
  ...
  const [formData, setFormData] = useState({
    name: '', designation: '', department: 'CS', qualification: '', experience: '', email: '', isHOD: false, order: 0
  });
  const [image, setImage] = useState(null);

  const departments = ['CS', 'Civil', 'Electrical', 'Mechanical', 'EC', 'Science', 'Academic Administration', 'Administration Staff'];
        ...
        {faculty.map(member => (
          <div key={member._id} className="bg-white p-4 rounded-2xl shadow-sm border flex gap-4">
             ...
             <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 mt-1 font-semibold">{member.department} Dept | {member.experience} Yrs Exp</p>
                <div className="flex gap-2 mt-4">
                   ...
                   <button onClick={() => deleteFaculty(member._id)} className="text-red-500 hover:bg-red-50 p-2 rounded-lg border transition"><FaTrash /></button>
                </div>
             </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100] p-4 overflow-y-auto">
           <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl my-8">
              <div className="bg-primary p-6 text-white flex justify-between items-center">
                 <h3 className="font-bold text-lg">Add/Edit Faculty Profile</h3>
              ...
              <form onSubmit={handleSubmit} className="p-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                 ...
                 <div>
                    ...
                    <input type="text" className="w-full px-4 py-2 border rounded-lg" value={formData.qualification} onChange={(e) => setFormData({...formData, qualification: e.target.value})} />
                 </div>
                 <div>
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Experience (Years)</label>
                    <input type="number" className="w-full px-4 py-2 border rounded-lg" value={formData.experience} onChange={(e) => setFormData({...formData, experience: e.target.value})} />
                 </div>
                 <div className="col-span-2">
                 ...
                 <div className="flex items-center gap-2">
                    <input type="checkbox" id="hod" checked={formData.isHOD} onChange={(e) => setFormData({...formData, isHOD: e.target.checked})} />
                    <label htmlFor="hod" className="text-sm font-bold text-gray-700">Is Head of Department?</label>
                 </div>
