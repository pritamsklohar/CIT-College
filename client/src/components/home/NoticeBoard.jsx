    ...
    <div ref={containerRef} className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 h-full">
      ...
      <div className="flex border-b">
        {tabs.map(tab => (
          ...
          >
            {tab === 'Media' ? 'CIT In Media' : `${tab} Notice`}
          </button>
        ))}
      </div>

      <div className="p-4 overflow-y-auto max-h-[450px] custom-scrollbar">
        {loading ? (
          <div className="space-y-4 p-4">
             {[1,2,3,4].map(i => <div key={i} className="h-16 bg-gray-100 animate-pulse rounded-lg"></div>)}
          </div>
        ) : notices.length > 0 ? (
          <div className="space-y-4">
            {notices.map((notice, index) => (
              <a
                ...
                className="notice-item flex items-start gap-4 p-4 rounded-xl hover:bg-gray-50 border border-transparent hover:border-gray-200 transition group"
              >
                <div className="bg-primary bg-opacity-10 text-primary p-3 rounded-lg flex flex-col items-center justify-center min-w-[65px] group-hover:bg-accent group-hover:text-white transition-colors">
                   <span className="text-xs font-bold uppercase">{new Date(notice.date).toLocaleString('default', { month: 'short' })}</span>
                   <span className="text-xl font-bold">{new Date(notice.date).getDate()}</span>
                </div>
                <div className="flex-1">
                  <h4 className="font-bold text-gray-800 line-clamp-2 mb-1 group-hover:text-primary">{notice.title}</h4>
